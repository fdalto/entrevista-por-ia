      ...
      if (resultado.isFinal) {
        try {
          ...
          try {
            ...
            debug(`falha na marcacao do segmento final, aplicando fallback '??': ${errorSegmento.message || String(errorSegmento)}`);
            marcarSegmentoIndefinido(texto);
          }
          adicionarTrechoConsolidado(prepararTextoTranscricaoFinal(textoBruto), "final");
          state.interimAtual = "";
...
async function executarLoopCiclicoCompat(controller) {
  while (state.entrevistaAtiva && controller && !controller.cancelled) {
    try {
      ...
      setStatus("Status: nao foi possivel iniciar recognition em ciclo curto.");
      debug(`Falha ao iniciar recognition em ciclos curtos: ${errorStart.message || String(errorStart)}`);
      state.entrevistaAtiva = false;
      atualizarEstadoControles();
      break;
    }
    ...
    await pararRecognitionSemFlushCompat("cycle");
...
async function iniciarEntrevista() {
  ...
  try {
    ...
    state.entrevistaAtiva = true;
    atualizarEstadoControles();
    setStatus("Status: entrevista ativa com ciclos curtos de recognition.");
    state.interviewLoopCompat = {
      cancelled: false,
      track: sessaoEntrevista.sttTrack
    };
    executarLoopCiclicoCompat(state.interviewLoopCompat).catch((errorLoop) => {
      debug(`Loop ciclico falhou: ${errorLoop.message || String(errorLoop)}`);
    });
    debug("Entrevista iniciada.");
  } catch (error) {
    if (sessaoEntrevista) {
