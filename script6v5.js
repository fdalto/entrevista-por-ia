...
function processarResultadoFinalCompat(textoBruto) {
  ...
  if (isMobile() && state.entrevistaAtiva) {
    ...
    debug(`trecho final pendente para analise serial mobile: ${texto}`);
  } else {
    try {
      fecharSegmento(texto);
    } catch (error) {
...
async function executarLoopSerialCompat(controller) {
  while (state.entrevistaAtiva && controller && !controller.cancelled) {
    try {
      iniciarRecognition({
      ...
      debug(`Falha ao iniciar recognition no modo serial mobile: ${errorStart.message || String(errorStart)}`);
      state.entrevistaAtiva = false;
      atualizarEstadoControles();
      break;
    }

    await waitCompat(EXPERIMENTAL_VARIANT.recognitionWindowMs);
    if (!state.entrevistaAtiva || controller.cancelled) {
      break;
    }

    await pararRecognitionSemFlushCompat("serial-window");
    if (!state.entrevistaAtiva || controller.cancelled) {
      break;
    }
    ...
    try {
      const frames = await capturarFramesTemporariosCompat(EXPERIMENTAL_VARIANT.analysisWindowMs, {
        logLabel: "Janela serial mobile"
      });
      incorporarFramesNaTimelineCompat(frames);
      processarPendenciasCompat();
    } catch (errorCapture) {
      debug(`Falha ao capturar janela serial mobile: ${errorCapture.message || String(errorCapture)}`);
      esvaziarPendenciasComoIndefinidoCompat();
    }

    await waitCompat(EXPERIMENTAL_VARIANT.cooldownMs);
  }
}
...
async function iniciarEntrevista() {
  ...
  state.entrevistaAtiva = true;
  atualizarEstadoControles();
  setStatus("Status: entrevista ativa em modo serial mobile.");
  state.interviewLoopCompat = { cancelled: false };
  executarLoopSerialCompat(state.interviewLoopCompat).catch((errorLoop) => {
    debug(`Loop serial mobile falhou: ${errorLoop.message || String(errorLoop)}`);
  });
}
...
async function finalizarEntrevista() {
  ...
  state.entrevistaAtiva = false;
  if (state.interviewLoopCompat) {
    state.interviewLoopCompat.cancelled = true;
  }
  atualizarEstadoControles();
...
EXPERIMENTAL_VARIANT.id = "v5";
EXPERIMENTAL_VARIANT.recognitionWindowMs = 3800;
EXPERIMENTAL_VARIANT.analysisWindowMs = 1200;
EXPERIMENTAL_VARIANT.cooldownMs = 300;

function obterMimeTypeRecorderV5() {
  ...
  for (let i = 0; i < candidatos.length; i += 1) {
    if (MediaRecorder.isTypeSupported && MediaRecorder.isTypeSupported(candidatos[i])) {
      return candidatos[i];
    }
  }
  return "";
...
async function gravarBlobAudioV5(duracaoMs) {
  if (typeof MediaRecorder === "undefined") {
    throw new Error("MediaRecorder nao disponivel no navegador.");
  }
  const stream = await navigator.mediaDevices.getUserMedia({
    audio: obterAudioConstraintsCompat()
  });
  const mimeType = obterMimeTypeRecorderV5();
  const partes = [];
  let recorder = null;
  try {
  ...
  await new Promise((resolve, reject) => {
    recorder.onerror = (event) => {
      reject(new Error(event.error && event.error.message ? event.error.message : "falha ao gravar audio"));
    };
  ...
  return new Blob(partes, { type: mimeType || (partes[0] && partes[0].type) || "audio/webm" });
}

function calcularSpectralCentroidFrameV5(buffer, sampleRate) {
  ...
  return clamp((centroid - CENTROID_MIN_HZ) / (CENTROID_MAX_HZ - CENTROID_MIN_HZ), 0, 1);
}

async function extrairFramesDoBlobV5(blob) {
  const AudioContextCtor = window.AudioContext || window.webkitAudioContext;
  if (!AudioContextCtor) {
    throw new Error("AudioContext nao disponivel para decodificar blob.");
  }
  ...
  try {
    ...
    const hop = Math.max(256, Math.floor(audioBuffer.sampleRate * (FEATURE_INTERVAL_MS / 1000)));
    const frames = [];
    for (let inicio = 0; inicio + frameSize <= left.length; inicio += hop) {
      const trechoL = left.subarray(inicio, inicio + frameSize);
      const trechoR = right.subarray(inicio, inicio + frameSize);
      const rmsL = rms(trechoL);
      const rmsR = rms(trechoR);
      const mix = criarSinalMixado(trechoL, trechoR);
      frames.push({
        ch: clamp((rmsL - rmsR) / (rmsL + rmsR + 1e-6), -1, 1),
        vol: clamp((rmsL + rmsR) * 0.5, 0, 1),
        pit: detectarPitchNormalizado(mix, audioBuffer.sampleRate),
        zcr: calcularZeroCrossingRate(mix),
        cent: calcularSpectralCentroidFrameV5(mix, audioBuffer.sampleRate)
      });
    }
    return frames;
  } finally {
...
async function capturarFramesTemporariosCompat(duracaoMs, opcoes = {}) {
  const blob = await gravarBlobAudioV5(duracaoMs);
  const frames = await extrairFramesDoBlobV5(blob);
  if (opcoes.logLabel) {
    debug(`${opcoes.logLabel}: ${frames.length} frames extraidos de chunk local.`);
  }
  return frames;
}
...
async function calibrar(nome) {
  ...
  try {
    ...
    setStatus(`Status: capturando nome de ${nome} com SpeechRecognition...`);
    textoCalibracao = await capturarTranscricaoCalibracao(CALIBRACAO_MS);
    nomeExtraido = extrairNomeDaCalibracao(textoCalibracao);
    if (!nomeExtraido) {
      setStatus("Status: nome nao identificado na calibracao; usando rotulo padrao.");
    ...
    setStatus(`Status: gravando chunk local para assinatura de ${nome}...`);
    const framesBrutos = await capturarFramesTemporariosCompat(CALIBRACAO_MS, {
      logLabel: `Calibracao recorder ${nome}`
    });
    const diagnosticoFrames = classificarFramesCalibracao(framesBrutos.slice());
    const framesVoz = diagnosticoFrames.validos;
    if (framesVoz.length < CALIBRACAO_MIN_FRAMES_VOZ) {
...
async function iniciarEntrevista() {
  ...
  state.entrevistaAtiva = true;
  atualizarEstadoControles();
  setStatus("Status: entrevista ativa com gravacao local em chunks.");
  state.interviewLoopCompat = { cancelled: false };
  executarLoopSerialCompat(state.interviewLoopCompat).catch((errorLoop) => {
    debug(`Loop serial com MediaRecorder falhou: ${errorLoop.message || String(errorLoop)}`);
  });
}
...
async function finalizarEntrevista() {
  ...
  if (state.pendingFinalSegmentsCompat.length) {
    try {
      const frames = await capturarFramesTemporariosCompat(EXPERIMENTAL_VARIANT.analysisWindowMs, {
        logLabel: "Chunk final de encerramento"
      });
      incorporarFramesNaTimelineCompat(frames);
      processarPendenciasCompat();
    } catch (errorFinal) {
      debug(`Falha ao processar chunk final de encerramento: ${errorFinal.message || String(errorFinal)}`);
      esvaziarPendenciasComoIndefinidoCompat();
    }
  }
